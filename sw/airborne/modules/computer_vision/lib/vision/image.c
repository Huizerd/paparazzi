                           ...
                           const uint8_t *from_color, const uint8_t *line_color, const uint8_t *to_color)
{
  ...
  for (uint16_t i = 0; i < points_cnt; i++) {
    ...
    struct point_t to = {
      .x = (uint32_t)roundf(((float)vectors[i].pos.x + vectors[i].flow_x * 5.0f) / subpixel_factor),
      .y = (uint32_t)roundf(((float)vectors[i].pos.y + vectors[i].flow_y * 5.0f) / subpixel_factor)
    };
